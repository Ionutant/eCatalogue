...
@Override
public int addGrade(Grades grades) {
    String sql = "INSERT INTO grades (grade_id, student_id, course_id, total_points, earned_points, status, submission_content, feedback) " +
            "VALUES (DEFAULT, ?, ?, ?, ?, ?, ?, ?) RETURNING grade_id ;";
    return dao.queryForObject(sql, Integer.class, grades.getStudentId(), grades.getCourseId(), grades.getTotalPoints(), grades.getEarnedPoints(), grades.getStatus(), grades.getSubmissionContent(), grades.getFeedback());
}

    @Override
    public List<Grades> getAllGrades() {
        List<Grades> gradesList = new ArrayList<>();
        String sql = "SELECT grade_id, student_id, course_id, total_points, earned_points, status, submission_content, feedback " +
                "FROM grades;";
        SqlRowSet result = dao.queryForRowSet(sql);
        while (result.next()) {
            Grades grades = mapRowToGrades(result);
...
//    }
    ...
    @Override
    public Grades getGrade(int studentId) {
        Grades grades = null;
        String sql = "SELECT grade_id, student_id, course_id, total_points, earned_points, status, submission_content, feedback " +
                "FROM grades WHERE student_id = ?;";
        SqlRowSet result = dao.queryForRowSet(sql, studentId);
        while (result.next()) {
            grades = mapRowToGrades(result);
        }
        return grades;
    ...
    @Override
    public void updateGrade(Grades grades) {
        String sql = "UPDATE grades SET student_id = ?, course_id = ?, total_points = ?, earned_points = ?, status = ?, submission_content = ?, feedback = ? " +
                "WHERE grade_id = ?;";
        dao.update(sql, grades.getStudentId(), grades.getCourseId(), grades.getTotalPoints(), grades.getEarnedPoints(), grades.getStatus(), grades.getSubmissionContent(), grades.getFeedback(), grades.getGradeId());
    }
