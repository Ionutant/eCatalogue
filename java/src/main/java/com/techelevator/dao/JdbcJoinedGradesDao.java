...
//        return dao.queryForObject(sql, Integer.class, joinedGrades.getStudentId(), joinedGrades.getCourseId(), joinedGrades.getTotalPoints(), joinedGrades.getEarnedPoints(), joinedGrades.getSubmissionContent(), joinedGrades.getFeedback());
//    }
...
//    }
    ...
    public void updateJoinedGrade(JoinedGrades joinedGrades) {
        String sql = "UPDATE grades SET student_id = ?, course_id = ?, total_points = ?, earned_points = ?, status = ?, submission_content = ?, feedback = ? " +
                "WHERE grade_id = ?;";
        dao.update(sql, joinedGrades.getStudentId(), joinedGrades.getCourseId(), joinedGrades.getTotalPoints(), joinedGrades.getEarnedPoints(), joinedGrades.getStatus(), joinedGrades.getSubmissionContent(), joinedGrades.getFeedback(), joinedGrades.getGradeId());
    }
